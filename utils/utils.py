...
def load_corpus(dataset_str):
    ...
    # 将allx和tx两个矩阵垂直叠在一起,命名为features
    # tolil()是一个转换操作，将稀疏矩阵转换为lil_matrix类型。lil_matrix是一种稀疏矩阵的表示方式，它在进行某些操作时更加高效。
    features = sp.vstack((allx, tx)).tolil()
    # 将ally和ty两个数组垂直叠在一起, 命名为labels
    labels = np.vstack((ally, ty))
    print(len(labels))
    ...
    # sample_mask目的是创建一个布尔掩码，标识哪些数据点属于训练集。
    # 具体来说，它会创建一个与数据总数(labels.shape[0])相同长度的布尔数组，其中训练集索引对应的位置为True，而其他位置为False。
    # 这个掩码将用于在训练机器学习模型时选择训练集的样本。
    train_mask = sample_mask(idx_train, labels.shape[0])
    val_mask = sample_mask(idx_val, labels.shape[0])
    test_mask = sample_mask(idx_test, labels.shape[0])
...
def preprocess_adj(adj):
    """Preprocessing of adjacency matrix for simple GCN 1111 and conversion to tuple representation."""
